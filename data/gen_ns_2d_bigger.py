...
if __name__ == '__main__':
    ...
    for i, item in enumerate(seed):
        ...
        # sol.shape [1001,4096,1]

        # bigger domain size
        sol = sol.reshape(-1, x_num, y_num, sol.shape[-1])
        sol = torch.cat((sol, sol), dim=1)  # (-1,64*2,64,1)
        sol = torch.cat((sol, sol), dim=2)  # (-1,64*2,64*2,1)
        sol = sol.reshape(sol.shape[0], -1, sol.shape[-1])
        ...
        u_path = os.path.join(
            '/mnt/miyuan/AI4Physics/Data/{}/2d_{}_u_t{}_n{}_{}.npy'.format(dataset_name, dataset_name, record_steps + 1,
                                                                           node_num*factor**2, item))
        np.save(u_path, sol.cpu().numpy())

    for i, item in enumerate(seed):
        x = np.linspace(0 - pad_len * dx, x_max * factor + pad_len * dx, x_num * factor + 2 * pad_len + 1)
        y = np.linspace(0 - pad_len * dy, y_max * factor + pad_len * dy, y_num * factor + 2 * pad_len + 1)
    ...
    for i, item in enumerate(seed):
        ...
        force = force.reshape(x_num, y_num, force.shape[-1])
        force = torch.cat((force, force), dim=0)  # (64*2,64,1)
        force = torch.cat((force, force), dim=1)  # (64*2,64*2,1)
        force = force.reshape(-1, force.shape[-1])

        np.save(force_path, force.cpu().numpy())
