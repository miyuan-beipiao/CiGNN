...
class GaussianRF(object):
    ...
    def __init__(self, dim, size, alpha=2, tau=3, sigma=None, boundary="periodic", device=None):
        ...
        if dim == 1:
            ...
            # 反转
            k_y = wavenumers.transpose(0, 1)
            k_x = wavenumers
            # k_x = k_x[..., :k_max + 1].to(device)  # [01213456789...-1]
            # k_y = k_y[..., :k_max + 1].to(device)  # [000000000000000000000,11111111111111111111]
...
def navier_stokes_2d(w0, f, visc, T, delta_t=1e-4, record_steps=1):
    ...
    # fno将原来的方程中的ky和kx写反了，不知道是否有影响
    # k_x = torch.cat((torch.arange(start=0, end=k_max, step=1, device=w0.device),
    #                  torch.arange(start=-k_max, end=0, step=1, device=w0.device)), 0).repeat(N, 1)
    # # Wavenumbers in x-direction
    # k_y = k_x.transpose(0, 1)

    # 正常公式中的kx 和 ky
    N = torch.tensor(N)
    k_x = 1j * torch.ones((1, N)).T * (
            torch.remainder(torch.arange(1, N + 1) - torch.ceil(N / 2 + 1), N) - torch.floor(N / 2))
    k_y = 1j * (torch.remainder(torch.arange(1, N + 1).reshape(1, -1).T - torch.ceil(N / 2 + 1), N) - torch.floor(
        N / 2)) * torch.ones((1, N))
    ...
    dealias = torch.unsqueeze(
        torch.logical_and(k_y.real <= (2.0 / 3.0) * k_max, k_x.real <= (2.0 / 3.0) * k_max).float(), 0)
    ...
    for j in range(steps):
        ...
        F_h = torch.fft.rfft2(u * w_x + v * w_y)

        # Dealias
        F_h = dealias * F_h

        # Crank-Nicolson update 1
        # w_h = (-delta_t * F_h + delta_t * f_h + (1.0 - 0.5 * delta_t * visc * lap) * w_h) / (
        #         1.0 + 0.5 * delta_t * visc * lap)

        # Crank-Nicolson update 2
        w_h_update = ((1 / delta_t + 0.5 * visc * lap) * w_h - F_h + f_h) / (1 / delta_t - 0.5 * visc * lap)
...
def postProcess(w0, i):
    ...
    plt.contourf(x_star, y_star, u_pred, levels=80, cmap='jet')
    ax.axis('square')
...
if __name__ == '__main__':
    ...
    record_steps = 50  # Number of snapshots from solution
    ...
    for i, item in enumerate(seed):
        ...
        for _ in range(0, sol.shape[0], record_steps // 5):
